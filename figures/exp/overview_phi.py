...
if __name__ == '__main__':
    ...
    cpuonly_token_throughput, df_cpuonly = get_cpuonly_token_gen_throughput(
        '../../logs_phi/OMP_logs/OMP_*/token_gen_time.csv', nth)
    ...
    for i, cache_policy in enumerate(["LRU"]):
        ...
        x_position = positions + i * (bar_width) - (bar_width * (0.5 - offset))
    ...
    ax.set_title('Overall Token Generation Throughput of Phi3.5-MoE',
                 fontsize=fs)
    ...
    plt.savefig(os.path.join('output',
                             f'phi3.5_cpu_vs_gpu_throughput_{nth}.pdf'),
                format='pdf',
                bbox_inches='tight',
                dpi=600)
