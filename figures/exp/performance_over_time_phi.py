...
def draw(omp: int, show_plot=False, windows_size: int = 10):
    df_cpu = get_cpu_or_gpu_only_time('../../logs_phi', omp)
    df_gpu = get_cpu_or_gpu_only_time('../../logs_phi')
    df_collab = get_collab_token_gen_throughput('../../logs_phi/LRU', omp)

    # Apply a rolling mean to smooth the data
    LIMIT_ITEMS = 1800  # Limit the number of items to plot
    LIMIT_ITEMS = min(len(df_cpu), len(df_gpu), len(df_collab), LIMIT_ITEMS)
    ...
    fig.savefig(os.path.join(output_dir,
                             f'phi_gen_time_history_OMP_{omp}.pdf'),
                bbox_inches='tight',
                format='pdf')
    if show_plot:
        plt.show()
