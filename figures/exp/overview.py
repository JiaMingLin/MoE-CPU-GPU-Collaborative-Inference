...
# nth: index of the throughput value to retrieve
# return: token generation throughput
def get_cpu_token_gen_throughput(path: str,
                                 omp_num_thread: int,
                                 cache_policy :str,
                                 nth: int):
    # Directory containing the log files
    ...
    # /OMP_<omp_num_thread>
    # /token_gen_time.csv'
    log_dir = os.path.join(path, cache_policy, '*', f'OMP_{omp_num_thread}', 'token_gen_time.csv')
    ...
    for file in glob.glob(log_dir):
        # CPU_BS8_NWAYS7_logs
        cache_nblocks = int(os.path.basename(os.path.dirname(os.path.dirname(file))).split('_')[1][2:])
        cache_nways = int(os.path.basename(os.path.dirname(os.path.dirname(file))).split('_')[2][5:])
        data = []
        with open(file, 'r') as f:
            reader = csv.reader(f)
            for row in reader:
        ...
        # Get the nth index from the DataFrame
        tokenGenThroughput.append([(cache_nblocks, cache_nways), df[cache_nways].iloc[nth-1]['Accumulated Generation Throughput (tokens/s)']])
...
if __name__ == '__main__':
    ...
    gpu_token_throughput = get_gpu_token_gen_throughput('../../logs/GPU_Offload_log/', nth)
    ...
    for omp_num_thread in omp_num_thread_list:
        for cache_policy in cache_policy_list:
            cpu_token_throughput, df_cpu = get_cpu_token_gen_throughput(log_base_dir, omp_num_thread, cache_policy, nth)
            df_obj[omp_num_thread][cache_policy] = cpu_token_throughput

    cpuonly_token_throughput, df_cpuonly = get_cpuonly_token_gen_throughput('../../logs/OMP_logs/OMP_*/token_gen_time.csv', nth)
    ...
    for omp_num_thread in omp_num_thread_list:
        for cache_policy in cache_policy_list:
            for (nblocks, throughput) in df_obj[omp_num_thread][cache_policy]:
                data.append((omp_num_thread, cache_policy, nblocks, throughput))
    ...
    # Sort the DataFrame
    df_plot = df_plot.sort_values(by=['OMP_NUM_THREADS', 'Cache_Policy', 'Nblocks'])
    ...
    for i, cache_policy in enumerate(["LRU"]):
        ...
        for j, nblocks in enumerate(subset['(Nblocks,Nways)'].unique()):
            ...
            for xx in subsubset['Throughput']:
                tmp.append(float(xx))
            x_position = positions + i * (bar_width) + jj * bar_width / pos_len - (bar_width*(0.5-0.06))
            ax.bar(
                x_position, 
                tmp, 
                width=bar_width / pos_len, 
