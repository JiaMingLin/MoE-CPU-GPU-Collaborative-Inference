...
gpu_token_throughput, df_gpu = get_gpu_token_gen_throughput(
    '../../logs/GPU_Offload_log/token_gen_time.csv', nth)

# Convert the list to a DataFrame
df_cpu_throughput = pd.DataFrame(cpu_token_throughput,
                                 columns=['OMP_NUM_THREADS', 'Throughput'])

# Convert the Throughput column to numeric
df_cpu_throughput['Throughput'] = pd.to_numeric(
    df_cpu_throughput['Throughput'])
...
plt.xticks(
    range(1,
          len(df_cpu_throughput) + 1),
    df_cpu_throughput['OMP_NUM_THREADS'])  # Set x-axis ticks from 1 to n

# Draw horizontal line for GPU throughput
plt.axhline(
    y=gpu_throughput_value,
    color='r',
    linestyle='--',
    label=f'Throughput of GPU offloading: {gpu_throughput_value:.2f} tokens/s')
plt.legend()
...
plt.savefig(os.path.join(output_dir, 'cpu_vs_gpu_throughput.pdf'),
            format='pdf',
            bbox_inches='tight')
